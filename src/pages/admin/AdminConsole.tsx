...
type UserRole = "ADMIN" | "COORDINADOR" | "LIDER DE AREA";
...
const AdminConsole: React.FC = () => {
  ...
  // --- estado local mock para gestión de usuarios (luego se conecta a API) ---
  ...
  // --- lista de escuelas únicas (sirve para filtros y para crear users) ---
  ...
  // --- handlers user form (mock, luego API) ---
  ...
  const handleCreateUser = (e: React.FormEvent) => {
    ...
    if (!userForm.email.endsWith("@cun.edu.co")) {
      setUserFormError("El correo debe ser institucional (@cun.edu.co).");
      return;
    }
    ...
    if (userForm.role !== "ADMIN" && !userForm.schoolName) {
      setUserFormError(
        "Coordinadores y líderes deben tener una escuela asignada."
      );
      return;
    }
    ...
    const newUser: AdminUserSummary = {
      ...
      role: userForm.role,
      schoolName:
        userForm.role === "ADMIN" ? "Todas" : userForm.schoolName || null,
      createdAt: new Date().toISOString(),
    };

    // TODO: aquí luego llamas a tu API POST /users
          ...
          <div className="flex flex-col gap-4">
            <div>
              ...
              <p className="text-neutral-400 mt-2 max-w-2xl text-sm leading-relaxed">
                Monitoreo del proceso de evaluación docente y gestión de
                usuarios responsables por escuela.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 mt-2">
              <button
                ...
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold border transition-all ${
                  activeTab === "DASHBOARD"
                    ? "bg-emerald-500/20 border-emerald-400 text-emerald-200 shadow-[0_0_20px_rgba(16,185,129,0.35)]"
                    : "bg-white/5 border-white/10 text-neutral-400 hover:border-emerald-400/40 hover:text-emerald-200"
                }`}
              >
              ...
              <button
                ...
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold border transition-all ${
                  activeTab === "USERS"
                    ? "bg-cyan-500/20 border-cyan-400 text-cyan-200 shadow-[0_0_20px_rgba(34,211,238,0.35)]"
                    : "bg-white/5 border-white/10 text-neutral-400 hover:border-cyan-400/40 hover:text-cyan-200"
                }`}
              >
                <ShieldCheck size={14} />
                Usuarios y Roles
                            ...
                            return (
                              ...
                              >
                                <div className="flex justify-between items-start mb-3">
                                  ...
                                  <span className="text-[10px] font-bold bg-white/5 px-2 py-0.5 rounded text-neutral-400">
                                    {s.total}
                                  </span>
                                </div>
        ...
        {activeTab === "USERS" && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1">
              <div className="bg-[#0f1110] rounded-3xl border border-white/10 p-6 space-y-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-neutral-500">
                      Registra coordinadores y líderes por escuela. El acceso se
                      valida con Google y correo institucional.
                    </p>
                  </div>
                </div>
                ...
                <form className="space-y-4" onSubmit={handleCreateUser}>
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full bg-[#0a0a0a] border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder-neutral-600 focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500/60 outline-none"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      placeholder="Coordinador de Ingeniería"
                      className="w-full bg-[#0a0a0a] border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder-neutral-600 focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500/60 outline-none"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      className="w-full bg-[#0a0a0a] border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500/60 outline-none"
                    >
                      <option value="ADMIN">Admin Global</option>
                      <option value="COORDINATOR">Coordinador de Escuela</option>
                      <option value="LEADER">Líder de Evaluación</option>
                    </select>
                    <p className="text-[10px] text-neutral-500 mt-1">
                      El admin ve todas las escuelas; coordinadores y líderes
                      solo su escuela asignada.
                    </p>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleUserFormChange("schoolName", e.target.value)
                      }
                      disabled={userForm.role === "ADMIN"}
                      className="w-full bg-[#0a0a0a] border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500/60 outline-none disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                      <option value="">
                        {userForm.role === "ADMIN"
                          ? "No aplica para Admin Global"
                  ...
                  <button
                    type="submit"
                    className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 text-xs font-semibold uppercase tracking-wide text-black shadow-[0_0_25px_rgba(16,185,129,0.4)] hover:brightness-110 transition-all"
                  >
                    <UserPlus size={16} />
                    Registrar usuario
                  </button>
                ...
                <div className="text-[10px] text-neutral-500 border-t border-white/5 pt-3">
                  Nota: más adelante este formulario debe llamar al endpoint
                  seguro del backend (ej. POST /users) y solo el rol ADMIN
                  debería verlo.
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-2">
              <div className="bg-[#0f1110] rounded-3xl border border-white/10 overflow-hidden flex flex-col h-[650px]">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-[#0a0a0a]/50 custom-scrollbar">
                  {users.length === 0 && (
                    <div className="flex flex-col items-center justify-center h-full text-neutral-600 gap-3">
                      ...
                      <p className="text-sm text-center max-w-xs">
                        Aún no has registrado usuarios en esta sesión. Crea un
                        coordinador o líder para tu primera escuela.
                      </p>
                    </div>
                  )}
                  ...
                  {users.map((u) => (
                    <div
                      key={u.id}
                      className="flex items-center justify-between bg-[#050505] border border-white/10 rounded-xl px-4 py-3 hover:border-emerald-500/30 transition-all"
                    >
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-neutral-500 mt-1">
                          {u.role === "ADMIN"
                            ? "Admin Global"
                            : `${u.role === "COORDINATOR" ? "Coordinador" : "Líder"} · ${
                                u.schoolName ?? "Sin escuela"
                              }`}
                        </span>
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        <span className="text-[10px] px-2 py-1 rounded-full bg-white/5 border border-white/10 uppercase tracking-wide text-neutral-400">
                          {u.role}
                        </span>
                        {u.createdAt && (
